...
build-impl-%: build_%/Makefile
	@cmake --build build_$* -j $(NPROCS) --target pg-lecture

# test
test-impl-%: build-impl-%
	@cmake --build build_$* -j $(NPROCS) --target pg-lecture_unittest
	@cmake --build build_$* -j $(NPROCS) --target pg-lecture_benchmark
	@cd build_$* && ((test -t 1 && GTEST_COLOR=1 PYTEST_ADDOPTS="--color=yes" ctest -V) || ctest -V)
	@pep8 tests

# testsuite service runner
service-impl-start-%: build-impl-%
	./build_$*/tests/runtests-pg-lecture-testsuite \
...
install-debug: build-debug
	@cd build_debug && \
		cmake --install . -v --component pg-lecture
...
--debug-start-in-docker: install
	@sed -i 's/config_vars.yaml/config_vars.docker.yaml/g' /home/user/.local/etc/pg-lecture/static_config.yaml
	@psql 'postgresql://user:password@service-postgres:5432/pg-lecture_db-1' -f ./postgresql/data/initial_data.sql
	@/home/user/.local/bin/pg-lecture \
		--config /home/user/.local/etc/pg-lecture/static_config.yaml
...
docker-impl-%:
	docker-compose run --rm pg-lecture-service make $*

# Build and runs service in docker environment
docker-start-service-debug:
	@docker-compose run -p 8080:8080 --rm pg-lecture-service make -- --debug-start-in-docker-debug

# Build and runs service in docker environment
docker-start-service:
	@docker-compose run -p 8080:8080 --rm pg-lecture-service make -- --debug-start-in-docker
